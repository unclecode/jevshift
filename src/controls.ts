...
const help='Use /jevshift status, setup, observe, auto, off, pin <model> [effort], or effort <low|medium|high|xhigh|max|auto|native>.';
export class SessionControls {
  ...
  async bind(id:string,source:string,host:ControlHost):Promise<void>{
    ...
    this.actual=undefined;this.nativeModel='unknown';this.nativeEffort=undefined;this.nativeEffortSeen=false;this.catalog={};this.catalogPending=undefined;
    ...
    const pending=(async()=>{
      ...
      if(s?.version===1&&s.sessionId===id&&['observe','off','auto','pin'].includes(s.mode??'')){
        ...
        else if(s.mode==='pin'&&(modelId(s.pin)||(s.pin===undefined&&effort!==undefined))){this.change('pin',s.pin,effort);this.blocked=s.blocked===true;}
  ...
  private async discover():Promise<Catalog>{
    ...
    try{const catalog=await pending;if(this.sessionId!==session)throw Error('Session changed');this.catalog=this.capped(catalog);return this.catalog;}
  ...
  private capped(catalog:Catalog):Catalog{
    const result:Catalog={};for(const tier of TIERS){const target=catalog[tier];if(!target)continue;
      ...
      if(efforts.length)result[tier]={...target,efforts};}return result;
  }
  ...
  async command(args:string,native:string):Promise<string>{
    ...
    if(action==='setup'&&words.length===1){
      ...
      try{
        ...
        this.catalog=this.capped(discovered);this.change('observe');await this.save();
        return `Models discovered: ${TIERS.filter(t=>this.catalog[t]).map(t=>`${t} = ${this.catalog[t]!.model} (${this.catalog[t]!.efforts.join(', ')})`).join('; ')}. Observe selected. Automatic effort cap: ${this.maxEffort}.`;
    ...
    if(action==='pin'&&(words.length===2||words.length===3)){
      ...
      this.change('pin',target.target.model,words[2] as Effort|undefined);await this.save();return this.status();
    }
    if(action==='effort'&&words.length===2){
      ...
      if(this.mode==='pin'&&isEffort(effort)){
        const target=resolveTarget(this.catalog,this.pin??native);
  ...
  private fallback(id:Boundary):ModelSelection{
    ...
    const model=pinned?this.pin??id.nativeModel:id.nativeModel,effort=fixed?this.effort as Effort:id.effort;
    if(pinned||fixed){
      const target=resolveTarget(this.catalog,model);
  ...
  async guard(id:Boundary):Promise<ModelSelection|undefined>{
    ...
    if(!this.blocked){
      ...
      const target=resolveTarget(this.catalog,this.pin??id.nativeModel);
    ...
    return this.track({model:this.pin??id.nativeModel,effort:this.effort==='native'?id.effort:this.effort,stopFallback:true},id,true);
  }
  async select(snapshot:ContextSnapshot,id:Boundary,host:ObserveHost):Promise<ModelSelection|undefined>{
    ...
    if(this.mode==='auto'||(this.mode==='pin'&&this.effort==='auto')){
      ...
      if(revision!==this.revision)return this.guard(id);
      const pinned=this.mode==='pin'?resolveTarget(this.catalog,this.pin??id.nativeModel):undefined;
  ...
  async finalize(selected:ModelSelection|undefined,id:Boundary):Promise<ModelSelection|undefined>{
    ...
    if(this.mode==='pin'&&!selected)return this.track(this.fallback(id),id,true);
    if(selected?.valid&&!selected.valid())return this.track(this.fallback(id),id,this.mode==='pin');
    return selected;
  }
