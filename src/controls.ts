...
import {compatible,resolveTarget,modelId,TIERS} from './catalog.ts';
...
export type SavedControl={version:1;sessionId:string;mode:Mode;pin?:string;blocked?:boolean};
...
const help='Use /jevshift status, setup, observe, auto, off, or pin <sonnet|opus|fable|model-id>.';
export class SessionControls {
  ...
  constructor(private allowAuto=false,private record:(event:Observation|RoutingEvent)=>void=()=>{}) {
    this.observer=new ObserveSession(e=>{
      if(e.event==='finished'&&e.sessionId===this.sessionId&&e.outcome!=='stale'&&e.outcome!=='aborted'){
        this.lastEvaluation=e;if(e.recommendation)this.recommended=e.recommendation;
      }
      this.record(e);
    });
  }
  invalidate():void{this.revision++;this.observer.invalidate();this.auto?.invalidate();}
  private change(mode:Mode,pin?:string):void {
    this.invalidate();this.auto?.stop();this.auto=undefined;this.observer.invalidate(true);
    this.mode=mode;this.pin=pin;this.blocked=false;this.note='';this.recommended=undefined;this.lastEvaluation=undefined;
    this.requested=undefined;
    if(mode==='auto'&&Object.keys(this.catalog).length)this.auto=new AutoSession(this.catalog,e=>{this.record(e)},this.observer);
  }
  async bind(id:string,source:string,host:ControlHost):Promise<void>{
    ...
    this.actual=undefined;this.nativeModel='unknown';this.catalog={};this.catalogPending=undefined;
    ...
    const pending=(async()=>{
      ...
      if(s?.version===1&&s.sessionId===id&&['observe','off','auto','pin'].includes(s.mode??'')){
        if(s.mode==='auto'&&!this.allowAuto){this.change('off');this.note='Saved auto mode is unavailable until experimental auto is enabled.';}
        else if(s.mode==='pin'&&modelId(s.pin)){this.change('pin',s.pin);this.blocked=s.blocked===true;}
        else if(s.mode!=='pin')this.change(s.mode as Mode);
      }
      this.loaded=true;
    })();
  ...
  private async discover():Promise<Catalog>{
    ...
    try{const catalog=await pending;if(this.sessionId!==session)throw Error('Session changed');this.catalog=catalog;return catalog;}
  ...
  async save():Promise<void>{
    ...
    const value:SavedControl={version:1,sessionId:id,mode:this.mode,...(this.pin?{pin:this.pin}:{}),...(this.blocked?{blocked:true}:{})};
  ...
  async command(args:string,native:string):Promise<string>{
    ...
    if(action==='setup'&&words.length===1){
      const revision=this.revision;
      try{
        const discovered=await this.host!.discover();
        if(revision!==this.revision)return 'Setup discarded because controls changed.';
        this.catalog=discovered;this.change('observe');await this.save();
        return `Models discovered: ${TIERS.filter(t=>discovered[t]).map(t=>`${t} = ${discovered[t]!.model}`).join(', ')}. Observe selected. Verified effort: low.`;
    ...
    if(action==='pin'&&words.length===2){
      ...
      const target=resolveTarget(this.catalog,words[1]);
      if(!target)return 'That model is not in the supported native catalog; existing mode unchanged.';
      if(!compatible(target.target,native,undefined))return 'That pin would reduce the selected context window; existing mode unchanged.';
      this.change('pin',target.target.model);await this.save();return this.status();
    }
  ...
  status():string{
    ...
    return [`JevShift: ${this.mode}${this.pin?` (${this.pin})`:''}${this.blocked?' — blocked; choose another model or /jevshift off':''}`,
      `Native default: ${this.nativeModel}`,`Last Jev result: ${evaluationText}`,`Last Jev recommendation: ${this.recommended??'none'}`,
      `Last requested model: ${this.requested??'none'}`,`Last returned model: ${this.actual??'none'}`,
      ...(this.note?[this.note]:[]),...(this.mode==='pin'?['Pin makes no Jev calls. Changes take effect on the next model request; use low effort.']:[])].join('\n');
  }
  ...
  async guard(id:Boundary):Promise<ModelSelection|undefined>{
    ...
    if(!this.blocked){
      ...
      const target=resolveTarget(this.catalog,this.pin??'');
      if(!target||!compatible(target.target,id.nativeModel,id.effort)){this.blocked=true;this.note='Pinned model is unavailable or incompatible with this effort/context setting.';}
    }
    ...
    return this.track({model:this.pin!,effort:id.effort,stopFallback:true},id,true);
  }
  async select(snapshot:ContextSnapshot,id:Boundary,host:ObserveHost):Promise<ModelSelection|undefined>{
    ...
    if(this.mode==='auto'){
      ...
      if(revision!==this.revision)return this.guard(id);
      this.auto??=new AutoSession(this.catalog,e=>this.record(e),this.observer);
      // A restored auto controller is rebuilt after current-process discovery.
      selected=await this.auto.select(snapshot,id,host);
    }else await this.observer.consume(snapshot,id,host);
    if(this.revision!==revision)return this.guard(id);
    return this.track(selected??{model:id.nativeModel,effort:id.effort},id,false,this.mode==='auto');
  }
  async finalize(selected:ModelSelection|undefined,id:Boundary):Promise<ModelSelection|undefined>{
    ...
    if(this.mode==='pin'||this.mode==='off')return this.guard(id);
    if(selected?.valid&&!selected.valid())return this.track({model:id.nativeModel,effort:id.effort},id);
    return selected;
  }
  private track(selection:ModelSelection,id:Boundary,pinned=false,automatic=false):ModelSelection{
    ...
    this.requested=selection.model;
